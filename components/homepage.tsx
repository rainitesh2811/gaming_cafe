...
export function Homepage({ onLogout, isGoogleUser }: HomepageProps) {
  ...
  const [googlePasswordCreated, setGooglePasswordCreated] = useState(false)
  ...
  async function handlePasswordSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setGooglePasswordCreated(true)
      setPassword('')
    } catch (error) {
      setPasswordError(error instanceof Error ? error.message : 'Unable to save password')
    } finally {
  ...
  return <main className="home-shell">
    <header className="topbar" ref={topbarRef}><button className="icon-button" aria-label="Open menu" onClick={() => { setMenuOpen(!menuOpen); setProfileOpen(false); setNotificationsOpen(false) }}><Menu size={22} /></button><button className="location" type="button" onClick={() => setLocationRequest((request) => request + 1)}><MapPin size={16} /><span><small>YOUR LOCATION</small>{locationLabel}</span><ChevronDown size={15} /></button><div className="top-actions"><div className="notification-wrap"><button className="icon-button notification" aria-label="Notifications" aria-expanded={notificationsOpen} onClick={handleNotificationsClick}><Bell size={19} />{hasUnreadNotifications && <i />}</button>{notificationsOpen && <div className="notification-popover" role="status"><strong>Notifications</strong><p>{notificationsLoading ? 'Checking for new notifications...' : notificationMessage}</p></div>}</div></div>{menuOpen && <div className="menu-popover">{profileOpen ? <section className="profile-section"><button className="back-button" type="button" onClick={() => setProfileOpen(false)}>← Menu</button><strong>Profile</strong>{isGoogleUser && !googlePasswordCreated ? <><p className="profile-note">Signed in with Google. Create a password to also log in with your email.</p><form className="profile-form" onSubmit={handlePasswordSubmit}><label>Create password<input type="password" minLength={8} value={password} onChange={(event) => { setPassword(event.target.value); setPasswordSaved(false); setPasswordError('') }} placeholder="At least 8 characters" required /></label>{passwordError && <p className="auth-error" role="alert">{passwordError}</p>}<button className="profile-save" type="submit" disabled={passwordSaving}>{passwordSaving ? 'Saving password...' : passwordSaved ? 'Password saved' : 'Create password'}</button></form></> : <p className="profile-note">You are signed in with your account password.</p>}</section> : <><strong>Menu</strong><a href="#events">Events</a><a href="#cafes">Saved cafes</a><a href="#help">Help center</a><button className="menu-button" type="button" onClick={() => setProfileOpen(true)}><UserRound size={15} /> Profile</button><button className="menu-button logout-button" type="button" onClick={onLogout}>Log out <span>→</span></button></>}</div>}</header>
    <div className="home-content">
      ...
      <section className="events-section" id="events"><div className="section-heading"><div><p className="eyebrow">DON&apos;T MISS OUT</p><h2>Upcoming events</h2></div><a href="#all-events">View all <span>→</span></a></div><div className="event-carousel">{eventsLoading ? <article className="event-card event-status">Loading upcoming events...</article> : eventsError ? <article className="event-card event-status">Events are temporarily unavailable.</article> : events.length > 0 ? events.map((event) => <EventCard event={event} key={event.id} />) : <><article className="event-card event-status">No upcoming events yet.</article><article className="event-card event-status event-status-secondary">More events are coming soon.</article></>}</div></section>
